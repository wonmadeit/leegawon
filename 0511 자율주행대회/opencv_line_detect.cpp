...
#define ROI_CENTER_Y  300 //200
#define ROI_WIDTH     60 //60 roi box, y width/2
...
int main(int argc, char **argv)
{
   ...
   */
   ...
   float  steering_conversion = 0.35;// 0.25;//0.15;//0.07; // Line pixel -> Angle parameter
   ...
   { 
     ...
     */
     ...
     mat_image_roi = Region_of_Interest_crop(mat_image_org_gray,points);    // Extract ROI     
	  ...
	  for(int i=0; i<linesP.size();i++)
	  {
		...
		*/
		...
		//printf("H :[%3d , %3d] , [%3d , %3d] \n", cx1,cy1, cx2,cy2);
	     ...
	     //gwlee sky blue roi line 
     if(linesP.size() != 0) 
     {
        
        line_center_x = line_center_x / (float)linesP.size() - img_width/2; // average
